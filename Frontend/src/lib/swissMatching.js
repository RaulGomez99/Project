...
function getMatchups(options, round, participants, matches) {
  matches = matches.filter(match => match.round < round)
  //participants = participants.filter(participant=> !participant.droppedOut)
  var mappings = getMappings(participants, matches)
  ...
  for(var i of sortedKeys) {
    if(results[i] !== -1 && !matchups.reduce((n, r) => n || r.home === mapIds.get(results[i]) || r.away === mapIds.get(results[i]),false)) {
        const playerOneWhites = mapIds.get(i) ? calcularBlanco(participants,matches).filter(a=>a[0]==mapIds.get(i))[0][1] : -1;
        const playerTwoWhites = mapIds.get(results[i]) ?calcularBlanco(participants,matches).filter(a=>a[0]=mapIds.get(results[i]))[0][1] : -1;
        if(playerOneWhites>playerTwoWhites){
            matchups.push({
                home: mapIds.get(results[i]),
                away: mapIds.get(i)
            })
        }else{
            matchups.push({
...
function getMappings(participants, matches) {
  return participants.filter(participant=>!participant.dropedOut).reduce((acc, participant) => {
...
function calcularBlanco(participants, matches){
    return participants.reduce((acc, participant) => {
        acc.push(matches.filter(match => {
          return match.home.id === participant.id ||
            match.away.id === participant.id
