...
function getMatchups(options, round, participants, matches) {
  ...
  for(var i of sortedKeys) {
    if(results[i] !== -1 && !matchups.reduce((n, r) => n || r.home === mapIds.get(results[i]) || r.away === mapIds.get(results[i]),false)) {
        const playerOneWhites = calcularBlanco(participants,matches).filter(a=>a[0]==mapIds.get(i))[0][1];
        const playerTwoWhites = calcularBlanco(participants,matches).filter(a=>a[0]=mapIds.get(results[i]))[0][1];
